...
var FeatureComposer = class {
  ...
  toggle(id3, state) {
    ...
    try {
      ...
      this.logger.log(`Feature - ${id3} failed to ${state ? "enable" : "disable"}: ${e}`);
      console.error(`[front-matter-title] feature "${id3}" failed to ${state ? "enable" : "disable"}`, e);
      if (state) {
        new import_obsidian4.Notice(
          `[Front Matter Title] Feature "${id3}" was disabled due to an error. See console for details.`
        );
        try {
          feature.disable();
        } catch (_) {
...
var ExplorerSort = class {
  ...
  tryToReplaceOriginalSort() {
    ...
    this.replacer = this.replacerFactory(
      ...
      function(feature, defaultArgs, vanilla) {
        var _a;
        return (_a = feature.getSortedFolderItems(defaultArgs == null ? void 0 : defaultArgs[0])) != null ? _a : vanilla.apply(this, defaultArgs);
      }
    );
    if (!this.replacer) {
  ...
  getSortedFolderItems(folder) {
    var _a;
    if (!((_a = this.view) == null ? void 0 : _a.fileItems)) {
    ...
    children.sort((a, b) => {
      var _a2, _b;
      const i = a instanceof import_obsidian5.TFolder;
      const r = b instanceof import_obsidian5.TFolder;
      let k;
      if (i && !r) {
        k = -1;
      } else if (!i && r) {
        ...
        const at = i ? a.name : (_a2 = this.resolver.resolve(a.path)) != null ? _a2 : a.name;
...
var TabManager = class extends AbstractManager {
  ...
  async doDisable() {
    ...
    (_a = this.replacer) == null ? void 0 : _a.disable();
    (_b = this.replacerDisplayText) == null ? void 0 : _b.disable();
    this.ref = null;
    this.reset();
    this.enabled = false;
  }
  ...
  initReplacer() {
    ...
    this.replacer = FunctionReplacer.tryCreate(proto, "setPinned", this, function(self2, [pinned], vanilla) {
      ...
      if (((_a2 = this == null ? void 0 : this.view) == null ? void 0 : _a2.getViewType()) === "markdown" /* MD */ && ((_b2 = this.view) == null ? void 0 : _b2.file)) {
        self2.innerUpdate(this.view.file.path);
      }
      return result;
    });
    (_a = this.replacer) == null ? void 0 : _a.enable();
    this.replacerDisplayText = FunctionReplacer.tryCreate(
      ...
      function(self2, _args, vanilla) {
        ...
        const filePath = (_g = (_f = (_b2 = (_a2 = this == null ? void 0 : this.view) == null ? void 0 : _a2.file) == null ? void 0 : _b2.path) != null ? _f : ((_e = (_d = (_c = this == null ? void 0 : this.view) == null ? void 0 : _c.getState) == null ? void 0 : _d.call(_c)) != null ? _e : {}).file) != null ? _g : null;
        return filePath ? (_h = self2.resolver.resolve(filePath)) != null ? _h : vanilla.call(this) : vanilla.call(this);
      }
    );
    (_b = this.replacerDisplayText) == null ? void 0 : _b.enable();
  }
  reset() {
    var _a;
...
var SuggestFeature = class extends AbstractFeature {
  ...
  createChooserReplacer(chooser) {
    var _a, _b;
    if (typeof ((_a = chooser == null ? void 0 : chooser.prototype) == null ? void 0 : _a.setSuggestions) !== "function") {
    ...
    (_b = this.replacer) == null ? void 0 : _b.enable();
  }
...
var WindowFrameFeature = class {
  ...
  updateTitle() {
    ...
    let text;
    if (e.view instanceof import_obsidian8.EditableFileView && e.getViewState().state.file) {
...
var AbstractBuilder = class {
  ...
  buildEnable() {
    ...
    return new import_obsidian9.Setting(this.context.getContainer()).setName(fragment).setDesc(this.options.desc).addButton((b) => {
  ...
  buildTemplates(el) {
    ...
    for (const type of ["main", "fallback"]) {
      ...
      const setting = new import_obsidian9.Setting(el).setName(t2(`template.${type}`)).setDesc(this.getDesc(type, tStorage.value())).addText((e) => {
        var _a;
...
var AliasBuilder = class extends AbstractBuilder {
  ...
  buildValidatorDropdown(el) {
    ...
    const s = new import_obsidian10.Setting(el).setName(t2("strategy"));
    this.validatorDropdown = new import_obsidian10.DropdownComponent(s.controlEl).addOptions({
  ...
  buildStrategyDropdown(el) {
    ...
    const s = new import_obsidian10.Setting(el).setName(t2("strategy"));
    this.strategyDropdown = new import_obsidian10.DropdownComponent(s.controlEl).addOptions({
...
var TemplatesBuilder = class extends AbstractBuilder2 {
  ...
  buildTemplate() {
    new import_obsidian11.Setting(this.container).setName(this.createDocFragment(t2("template.common.main.name"))).setDesc(t2("template.common.main.desc")).addText(
      (text) => {
        var _a;
  ...
  buildFallbackTemplate() {
    new import_obsidian11.Setting(this.container).setName(this.createDocFragment(t2("template.common.fallback.name"))).setDesc(t2("template.common.fallback.desc")).addText(
      (text) => {
        var _a;
...
var UtilBuilder = class {
  ...
  buildDebug(item, container) {
    new import_obsidian12.Setting(container).setName(t2("debug_info.title")).setDesc(t2("debug_info.desc")).addToggle((e) => e.setValue(item.value()).onChange((e2) => item.set(e2)));
  }
  buildBoot(item, container) {
    new import_obsidian12.Setting(container).setName(t2("boot_delay.title")).setDesc(t2("boot_delay.desc")).addText(
    ...
    new import_obsidian12.Setting(container).setName(t2("boot_background.title")).setDesc(t2("boot_background.desc")).addToggle(
...
var RulesDelimiterBuilder = class extends AbstractBuilder2 {
  ...
  doBuild() {
    this.setting = new import_obsidian13.Setting(this.container).setName(t2("rule.delimiter.name")).setDesc(t2("rule.delimiter.desc"));
    this.buildDropdown();
    this.buildText();
  }
  buildDropdown() {
    const enabled = this.item.get("enabled");
    new import_obsidian13.DropdownComponent(this.setting.controlEl).addOptions({ N: t2("rule.delimiter.first"), Y: t2("rule.delimiter.join") }).setValue(enabled.value() ? "Y" : "N").onChange((e) => {
      enabled.set(e === "Y");
  ...
  buildText() {
    ...
    this.text = new import_obsidian13.TextComponent(this.setting.controlEl).setValue(v.value()).setDisabled(!this.isEnabled()).setPlaceholder(this.getPlaceholder()).onChange((e) => v.set(e));
  }
  getPlaceholder() {
...
var RulesPathsBuilder = class extends AbstractBuilder2 {
  ...
  doBuild() {
    this.settings = new import_obsidian14.Setting(this.container).setName(t2("rule.path.name")).addDropdown(
      ...
      c.appendChild(
        createDiv(
          ...
          (e) => {
            ...
            new import_obsidian15.TextComponent(e).setValue((_a = value == null ? void 0 : value[i]) != null ? _a : "").onChange((e2) => value.splice(i, 1, e2)).onChanged();
          }
        )
      );
      c.appendChild(
...
var NoteLinkBuilder = class extends AbstractBuilder {
  ...
  buildStrategy(el) {
    new import_obsidian16.Setting(el).setName(t2("feature.noteLink.strategy.name")).setDesc(t2("feature.noteLink.strategy.desc")).addDropdown((e) => {
      var _a;
      e.addOptions({
  ...
  buildApproval(el) {
    new import_obsidian16.Setting(el).setName(t2("feature.noteLink.approval.name")).setDesc(t2("feature.noteLink.approval.desc")).addDropdown((e) => {
      e.addOptions({
...
var ExplorerBuilder = class extends AbstractBuilder {
  ...
  buildSort(el) {
    new import_obsidian17.Setting(el).setName(t2("feature.explorer.sort.name")).setDesc(t2("feature.explorer.sort.desc")).addToggle((e) => {
...
var ResultDomWrapper = class {
  ...
  processLookupItem(file, item) {
    ...
    const node = (_c = (_b = item == null ? void 0 : item.containerEl) == null ? void 0 : _b.firstElementChild) == null ? void 0 : _c.find(".tree-item-inner");
    if (!node) {
      return;
    }
...
Container2.bind(inversify_types_default["factory:replacer"]).toFunction(
  (t3, m, a, i) => FunctionReplacer.tryCreate(t3, m, a, i)
);
...
var PluginHelper = class {
   ...
   */
  static shouldShowReleaseNotice(previous, current) {
...
var MetaTitlePlugin = class extends import_obsidian21.Plugin {
  ...
  maybeShowReleaseNotice() {
    ...
    this.saveData(this.storage.collect()).catch((e) => {
      var _a;
      return (_a = this.logger) == null ? void 0 : _a.log(e);
    });
  }
  onunload() {
    this.fc.disableAll();
  ...
  async loadSettings() {
    ...
    this.previousVersion = loaded ? (_a = loaded.version) != null ? _a : "0.0.0" : null;
  ...
  reloadFeatures() {
    ...
    for (const [id3, state] of states) {
      try {
        ...
        this.logger.log(e);
        new import_obsidian21.Notice(e);
      }
    }
  }
  registerCommands() {
