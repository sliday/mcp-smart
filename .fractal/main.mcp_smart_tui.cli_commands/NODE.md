...
Implement Task 5 (init, doctor, and one-shot ask command functions) from
`docs/superpowers/plans/2026-08-26-mcp-smart-usability-tui.md`, using
...
- `src/commands/` (create and edit command modules here only)
- `src/__tests__/commands.test.ts`

Do not edit `src/cli.ts`, `src/__tests__/cli.test.ts`, package files, README,
routing/MCP files, TUI files, or any other project file. The parent will resolve
interface friction and wire these functions into the CLI after merge. Escalate
any needed parent or sibling change over radio instead of crossing ownership.

Implement these pure interfaces for the parent to consume:

- `runInit(options): Promise<CommandResult>`
- `runDoctor(options): Promise<DoctorResult>`
- `runAsk(task, options): Promise<ConsultationResult>`

Requirements:

- Return structured data; do not own CLI text/JSON formatting. Preserve enough
  data for the parent to render either contract.
- `init` references `OPENROUTER_API_KEY` and copyable environment/client setup,
  but never accepts, stores, or prints a key value.
- `doctor` checks Node, terminal, API-key presence, OpenRouter access, and the
  default route. When the key is absent it reports a stable `missing` check with
  an action, skips the network probe, and emits no stack or secret.
- `ask` rejects empty input with stable code `TASK_REQUIRED`, preserves task
  whitespace/indentation/fenced code exactly, uses `OpenRouterClient`, and
  returns its typed consultation result. Use process `OPENROUTER_API_KEY` by
  default while keeping dependencies injectable for tests.
- Errors must be typed and actionable without stacks or secrets in normal data.
- Do not make live provider calls in tests.

Use strict TDD for every behavior. Before editing each TypeScript test, inspect
the actual source/dependency modules and record verified imports, method names,
signatures, types, and any discrepancy in node memory. Add the smallest focused
test, run it to capture the expected RED, implement only enough production code,
then capture GREEN. Keep exact RED/GREEN commands and outcomes in node memory
and summarize them in the child outbox. Use `apply_patch` for every hand-written
edit.

## Completion Requirements

- Only the owned project files differ from the parent branch.
- Focused command tests pass:
  `npm test -- --run src/__tests__/commands.test.ts`.
- Adjacent routing tests and TypeScript build pass; run the inherited
  `$NODE_DIR/scripts/test.sh` before completion.
- Tests prove init environment guidance, missing-key doctor/no network probe,
  JSON-serializable results without stacks/secrets, empty-task `TASK_REQUIRED`,
  OpenRouterClient delegation, and exact multiline task preservation.
- Node memory records source preflight plus RED and GREEN evidence for each
  production behavior and contains no forward-looking terminal-state items.
- The child outbox reports interfaces delivered, focused/full test evidence,
  and the final commit hash.
- The worktree is clean, `git diff --check` passes, and the branch remains
  available. Run `fractal node finish` in the same iteration after satisfying
  these checks.
