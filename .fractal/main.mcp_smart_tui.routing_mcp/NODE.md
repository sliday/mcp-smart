...
Implement Tasks 2, 3, and 4 from
`docs/superpowers/plans/2026-08-26-mcp-smart-usability-tui.md`, in that order.
The binding product authority is
`docs/superpowers/specs/2026-08-26-mcp-smart-usability-tui-design.md`; use it
when the plan leaves room for judgment.

You exclusively own these project files:

- `src/contracts.ts`
- `src/openrouter.ts`
- `src/SmartAdvisorServer.ts`
- `src/__tests__/contracts.test.ts`
- `src/__tests__/openrouter.test.ts`
- `src/__tests__/SmartAdvisorServer.test.ts`
- `src/__tests__/integration.test.ts`

Do not edit any other project file. In particular, do not edit parent-owned
`package.json`, `package-lock.json`, `tsconfig.json`, `src/index.ts`,
`src/cli.ts`, `src/__tests__/cli.test.ts`, or `README.md`, and do not create
command or TUI files. Escalate interface friction to the parent through radio
instead of crossing ownership.

Use strict TDD per behavior. Before changing each TypeScript test, read the
production source and installed type declarations; record actual imports,
method names, signatures, and discrepancies in topical node memory and your
outbox. Capture focused RED before production edits, then minimum production
code, then focused GREEN. Use `apply_patch` for hand-written edits.

Consult current library documentation through Context7 before assuming MCP SDK,
Axios, or OpenRouter APIs. Discover callable tools first and make at most three
failed documentation calls. If Context7 is unavailable or reports quota/rate
limit, stop that call path, verify what you can from installed declarations and
tests, and immediately report the verified documentation blocker in your
outbox. Never guess silently.

Required contracts:

- `ConsultInput`, `ConsultationResult`, `ConsultationReceipt`,
  `SmartErrorDetails`, `ResolvedRoute`, `resolveRoute`, and
  `buildConsultationCacheKey` match the binding spec. Cache identity includes
  intent, prompt version, resolved model/preset/cost tier, allowed/excluded
  models, maximum tokens, task, and context. `fresh` bypasses reads.
- Default routing is Balanced `openrouter/auto`, medium tier, plugin ID exactly
  `auto-router`. Auto plugin data uses `allowed_models` and `excluded_models`
  inside the plugin object. Direct models omit the plugin. `session_id` is at
  the request root and headers include `X-OpenRouter-Metadata: enabled`.
- Response normalization preserves absent optional fields. Do not invent
  provider, cost, classification, fallback, usage, or request metadata.
- Stable errors cover missing key, authentication, credits, restrictions/no
  eligible model, unavailable provider, timeout, local/provider rate limits,
  and circuit breaker failures without normal-output stacks or secrets.
  Authentication, credit, restriction, quota, and rate-limit failures stop on
  the first response. Timeout and 503 attempts total at most three.
- `SmartAdvisorServer.run()`, `listTools()`, and `callTool(name, args)` remain
  public and source-compatible. Advertise only `consult`, `smart_doctor`, and
  `smart_status`; only `task` is required for `consult`. Accept all seven
  current aliases without advertising them and map them to canonical intents.
  Successful consultations provide readable `content` and typed
  `structuredContent`.
- Preserve the existing prompt content and input whitespace unless the binding
  spec explicitly replaces behavior. Never persist or print an API key,
  prompt, context, or authorization header.

Focused TDD commands:

```bash
npm test -- --run src/__tests__/contracts.test.ts
npm test -- --run src/__tests__/openrouter.test.ts
npm test -- --run src/__tests__/SmartAdvisorServer.test.ts src/__tests__/integration.test.ts
```

Run adjacent focused suites after each GREEN, then `npm run build` and
`git diff --check`. Keep exact RED/GREEN summaries in memory and post a concise
outbox milestone after each task, including discrepancies and test counts.

## Completion Requirements

- Only the seven owned project files differ from the dispatcher baseline.
- Tasks 2–4 are implemented with recorded focused RED and GREEN evidence.
- Default Auto shape, direct routes, session metadata, optional receipt fields,
  cache identity/fresh bypass, retry stopping, and stable errors have automated
  coverage.
- MCP tests prove exactly three advertised canonical tools, task-only required
  input, typed structured output, diagnostics, and seven callable hidden aliases.
- The three focused commands above pass, `npm run build` passes, and
  `git diff --check` passes.
- Node memory is current and contains no forward-looking terminal todo; the
  outbox contains a final concise evidence summary for the parent.
- The work is committed on the local child branch, the worktree is clean, and
  `fractal node finish --reason="routing, OpenRouter, and MCP contracts complete"`
  is run while the node is active.
