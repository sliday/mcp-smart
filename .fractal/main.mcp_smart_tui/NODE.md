...
Implement the approved MCP Smart usability release in:

- Plan: `docs/superpowers/plans/2026-08-26-mcp-smart-usability-tui.md`
- Binding spec: `docs/superpowers/specs/2026-08-26-mcp-smart-usability-tui-design.md`

Follow the plan task by task. Treat the spec as the authority when the plan leaves room for judgment. Record each ruling in node memory, including the choice, reason, and rework risk.

Use test-driven development for each behavior. Read the source module and verify imports, method names, signatures, and types before you create or modify a JavaScript or TypeScript test. Record discrepancies before editing the test. Run the focused test to capture the expected RED result, write the minimum production code, then capture GREEN. Keep the evidence in node memory and the relevant child outbox.

Use `apply_patch` for hand-written file edits. Keep diffs limited to the approved feature. Do not modify `AGENTS.md`, `.channels_cache_v2.json`, or `codedb.snapshot`. Preserve existing formatting outside changed sections. Never persist or print an API key, prompt, context, or authorization header.

Use the named technologies and APIs:

- Node.js 22, TypeScript ESM, React 19, and Ink 7.
- OpenRouter `openrouter/auto` with plugin ID `auto-router` for Auto requests. Do not substitute `openrouter/auto-beta`, another router, or a custom router.
- Use `allowed_models` and `excluded_models` inside the Auto plugin object. Send `session_id` at the request body root and `X-OpenRouter-Metadata: enabled` as a header.
- Use Ink 7's `render` option `alternateScreen: true` and set `exitOnCtrlC: false` so the app can implement the two-stage cancellation contract.
- Consult current library documentation through Context7 before making API assumptions. Stop after three failed documentation calls.

Keep `mcp-smart` with no arguments as stdio MCP. Load React and Ink only in the `tui` branch. Make `--help` and `--version` work without an API key or server construction. Use the process environment for `OPENROUTER_API_KEY`.

Run the work through this topology. Each child must use a local branch and inherit this node's scripts. Give each child a written `NODE.md` contract with its exact file ownership, focused tests, TDD evidence, and completion checks. Fit all child budgets and the parent's integration reserve inside this run's cap.

1. The parent implements Task 1 and commits the dispatcher boundary. The parent owns `package.json`, `package-lock.json`, `tsconfig.json`, `src/index.ts`, `src/cli.ts`, `src/__tests__/cli.test.ts`, `README.md`, and final integration.
2. Spawn `routing_mcp` after Task 1. It owns `src/contracts.ts`, `src/openrouter.ts`, `src/SmartAdvisorServer.ts`, `src/__tests__/contracts.test.ts`, `src/__tests__/openrouter.test.ts`, `src/__tests__/SmartAdvisorServer.test.ts`, and `src/__tests__/integration.test.ts`. Review its diff and test evidence before merging it.
3. Spawn `cli_commands` from the merged routing state. It owns `src/commands/` and `src/__tests__/commands.test.ts`. The parent wires the merged command functions into `src/cli.ts` and updates the parent-owned CLI tests.
4. Spawn `ink_tui` from the merged command state. It owns `src/tui/` and `src/__tests__/tui.test.tsx`. Review its keyboard behavior, narrow layout, partial Compare results, cancellation, and receipt rendering before merging it.
5. The parent completes Task 7, performs the broad spec review, and runs the final gate.

Do not let a child write a parent-owned file or a sibling's file. Resolve interface friction in the parent. Do not delete or retire Fractal branches or worktrees after merging.

Retry transient provider timeouts and 503 responses at most three total attempts. Stop on the first authentication, credit, model restriction, quota, or rate-limit response. If the Codex or documentation provider reports a quota or rate limit, stop that call path and post a written summary instead of retrying.

Keep the TUI useful at first launch. A keyboard user must see the selected route, current focus, request stage, result, error action, and receipt without reading documentation. Preserve task whitespace, indentation, and fenced code. Compare must retain successful answers when another advisor fails and must label both outcomes.

## Completion Requirements

- `mcp-smart`, with no arguments, still starts the stdio MCP server.
- `tui`, `init`, `doctor`, `ask`, `--help`, and `--version` implement the approved text and JSON contracts.
- The TUI exposes Consult, Compare, Doctor, Setup, and Last Receipt, supports the approved keyboard map, uses the alternate screen, and adapts below 80 columns.
- The MCP server advertises only `consult`, `smart_doctor`, and `smart_status`, requires only `task` for `consult`, and accepts all seven existing aliases without advertising them.
- Consultations default to Balanced `openrouter/auto` with the `auto-router` plugin, medium cost tier, and session IDs. Direct model IDs omit the Auto plugin.
- Successful CLI and MCP calls return the answer and a typed receipt. Missing OpenRouter fields stay absent. Cache identity covers every field named in the spec, and `fresh` bypasses cache reads.
- Stable errors cover missing key, authentication, credits, restrictions, unavailable providers, timeout, local and provider rate limits, and circuit breaker failures without exposing secrets or stacks in normal output.
- Automated tests prove source-compatible dispatch, Auto request shape, direct routes, usage and cost normalization, hidden aliases, JSON commands, multiline preservation, narrow rendering, cancellation, partial Compare success, and optional receipt fields.
- `README.md` documents Node 22, environment setup, doctor, TUI, one-shot ask, canonical MCP configuration, presets, receipts, and legacy compatibility with copyable commands.
- `bash $NODE_DIR/scripts/test.sh` passes from the node worktree with clean output.
- `git diff --check` passes, the branch contains no key or secret, and the worktree has no uncommitted project changes.
- Node memory contains RED and GREEN evidence for each production behavior and a final spec checklist. Each child branch remains available after merge.
